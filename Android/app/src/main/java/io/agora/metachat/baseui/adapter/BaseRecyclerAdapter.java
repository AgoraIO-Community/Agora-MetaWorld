package io.agora.metachat.baseui.adapter;
...
import io.agora.metachat.baseui.adapter.listener.OnItemChildClickListener;
import io.agora.metachat.baseui.adapter.listener.OnItemClickListener;
import io.agora.metachat.tools.BaseUiTools;

/**
 * 基础RecyclerView adapter
 */
public class BaseRecyclerAdapter<B extends ViewBinding, T, H extends BaseRecyclerAdapter.BaseViewHolder<B, T>> extends RecyclerView.Adapter<H> {

    public @NonNull
    List<T> dataList;

    private final OnItemClickListener<T> mOnItemClickListener;
    private final OnItemChildClickListener<T> mOnItemChildClickListener;
    ...
    public BaseRecyclerAdapter(@Nullable List<T> dataList, Class<H> viewHolderClass) {
        this(dataList, null, viewHolderClass);
    }


    public BaseRecyclerAdapter(@Nullable List<T> dataList, @Nullable OnItemClickListener<T> listener, Class<H> viewHolderClass) {
        this(dataList, listener, null, viewHolderClass);
    }

    public BaseRecyclerAdapter(@Nullable List<T> dataList, @Nullable OnItemClickListener<T> listener, @Nullable OnItemChildClickListener<T> itemChildListener, Class<H> viewHolderClass) {
    ...
    public B getViewBindingByReflect(@NonNull LayoutInflater inflater, @Nullable ViewGroup container) {
        ensureBindingClass();
        try {
            return BaseUiTools.getViewBinding(bindingClass, inflater, container);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public void ensureBindingClass() {
        if (bindingClass == null)
            bindingClass = BaseUiTools.getGenericClass(viewHolderClass, 0);
    }
