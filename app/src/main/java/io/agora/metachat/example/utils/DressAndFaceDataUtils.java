...
import io.agora.metachat.example.models.AaManifest;
import io.agora.metachat.example.models.DressItemResource;
import io.agora.metachat.example.models.DressResource;
...
public class DressAndFaceDataUtils {
    ...
    public void initData(String filePath) {
        ...
        if (null != mManifest) {
            ...
            if (!catalogHash.equals(MMKVUtils.getInstance().getValue(MetaChatConstants.MMKV_CATALOG_HAS, ""))) {
                Log.i(TAG, "initData unzipIcons");
                unzipIcons(filePath);
                MMKVUtils.getInstance().putValue(MetaChatConstants.MMKV_CATALOG_HAS, catalogHash);
            }
        }
    }
