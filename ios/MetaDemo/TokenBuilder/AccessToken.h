...
inline std::string AccessToken::Build() {
  ...
  crc_channel_name_ = (uint32_t)crc32(
      0, reinterpret_cast<Bytef*>(const_cast<char*>(channel_name_.c_str())),
      (int)channel_name_.length());
  crc_uid_ =
    (uint32_t)crc32(0, reinterpret_cast<Bytef*>(const_cast<char*>(uid_.c_str())),
        (int)uid_.length());
  PackContent content;
...
inline void AccessToken::SetTokenExpiredTs(uint32_t seconds) {
  message_.ts = (uint32_t)::time(NULL) + seconds;
}
}  // namespace tools
}  // namespace agora
