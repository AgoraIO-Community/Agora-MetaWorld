...
class IRtcEngineEventHandler {
  ...
  */
  ...
  virtual void onResourceInfoListUpdated(ResourceInfoList* resourceInfoList, RESOURCE_ERROR_TYPE code) {
    (void)resourceInfoList;
    (void)code;
  }

  virtual void onResourceInfoUpdated(const char* id, ResourceInfo resourceInfo, RESOURCE_ERROR_TYPE code) {
    (void)id;
    (void)resourceInfo;
    (void)code;
  }

  virtual void onDownloadResourceStateChanged(const char* id, RESOURCE_STATE_TYPE state,
                                  int progress, int reason) {
    (void)id;
    (void)state;
    (void)progress;
    (void)reason;
...
///@endcond
  ...
  */
  ...
  /*
   * Sync resource info list
   * @param ResourceConfig resource configuration
   * @return
   * - 0: Success.
   * - < 0: Failure.
   */
  virtual int syncResourceInfoList(const ResourceConfig& resourceConfig) = 0;

  /*
   * Download specific resource id, which is get from onResourceInfoListUpdated or onResourceInfoUpdated
   * @param id resource id
   * @param subPath in case of user need hierarchy folder to save specific bundle, root path is set in ResourceConfig.rootPath
   * @return
   * - 0: Success.
   * - < 0: Failure.
   */
  virtual int downloadResource(const char* id, const char* subPath) = 0;

  /*
   * Get local resource info
   * After a successful method call, the SDK triggers the onResourceInfoUpdated
   * @param id resource id
   * @return
   * - 0: Success.
   * - < 0: Failure.
   */
  virtual int getResourceInfo(const char* id) = 0;

  /*
   * Get local resource info list
   * After a successful method call, the SDK triggers the onResourceInfoUpdated
   * @param id resource id
   * @return
   * - 0: Success.
   * - < 0: Failure.
   */
  virtual int getResourceInfoList() = 0;
  /*
