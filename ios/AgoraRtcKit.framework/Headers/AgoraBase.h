...
enum INTERFACE_ID_TYPE {
  ...
  AGORA_IID_STATE_SYNC = 12,
  AGORA_IID_METACHAT_SERVICE = 13,
};
...
struct WatermarkRatio {
  ...
  struct ResourceConfig {
    const char* appid;  // RTM, must have
    const char* token;  // RTM, must have
    uid_t uid;  // uid in RTM token, must have
    const char* root_path;  // root path
    ResourceConfig() : appid(OPTIONAL_NULLPTR), token(OPTIONAL_NULLPTR), uid(0), root_path(OPTIONAL_NULLPTR) {}
  };
...
enum RESOURCE_STATE_TYPE {
  RESOURCE_STATE_IDLE = 0,
  RESOURCE_STATE_DOWNLOADING = 1,
  RESOURCE_STATE_DOWNLOADED = 2,
  RESOURCE_STATE_DOWNLOAD_FAILED = 3
};

enum RESOURCE_ERROR_TYPE {
  RESOURCE_ERROR_OK = 0,
  RESOURCE_ERROR_NOT_FOUND = 1,
  RESOURCE_ERROR_INVALID_TOKEN = 2,
  RESOURCE_ERROR_NEED_SYNC = 3
};
...
struct ResourceInfoList {
  ResourceInfo* infoList;
  int ResourceInfoListLength;

  ResourceInfoList(): ResourceInfoListLength(0){
  }
};
